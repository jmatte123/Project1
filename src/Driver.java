...
public class Driver
{
     ...
     public static void main(String[] args) throws FileNotFoundException
     {
          ...
          for (Cargo unit : myPorts.get(0).getOutbound().toArray(Cargo.class))
          {
          ...
          {
               //Check Attributes
               if (newPort.getOutbound().toArray(Cargo.class)[x].getDest().equals(myPorts.get(0).getOutbound().toArray(Cargo.class)[x].getDest()) &&
                       newPort.getOutbound().toArray(Cargo.class)[x].getTonnage() == myPorts.get(0).getOutbound().toArray(Cargo.class)[x].getTonnage())
               {
                    match++;
               }
          }
          ...
          int dupes = 0;
          for (Cargo newO : newPort.getOutbound().toArray(Cargo.class))
          {
               for (Cargo oldO : myPorts.get(0).getOutbound().toArray(Cargo.class))
               {
                    if (newO.equals(oldO))
                    {
                         dupes++;
